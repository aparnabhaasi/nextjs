...
): [number, number][] { // Define return type as an array of number tuples
  let i = 0;
  const series: [number, number][] = []; // Define type explicitly

  while (i < count) {
    const x = baseval;
    const y = Math.floor(Math.random() * (yrange.max - yrange.min + 1)) + yrange.min;

    series.push([x, y]); // Now TypeScript knows this is a tuple
    baseval += 86400000;
    i++;
  }
