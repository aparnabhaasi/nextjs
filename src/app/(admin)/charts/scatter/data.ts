...
function generateDayWiseTimeSeries(baseval: number, count: number, yrange: { max: number; min: number }): [number, number][] {
  let i = 0
  const series: [number, number][] = [] // Explicitly define the type as an array of number tuples

  while (i < count) {
    const x = baseval
    const y = Math.floor(Math.random() * (yrange.max - yrange.min + 1)) + yrange.min

    series.push([x, y]) // Now TypeScript understands that series is an array of [number, number] tuples
    baseval += 86400000
    i++
  }
  return series
