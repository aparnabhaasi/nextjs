...
const MapWithMarkers = ({ google }: MapContainerProps) => {
  ...
  const StreetViewMap = ({ google }: MapContainerProps) => {
    ...
    const activateStreetView = (position: { lat: number; lng: number }) => {
      if (mapRef.current && mapRef.current.map) {  // Ensure mapRef.current.map is available
        const mapObj = mapRef.current.map.getStreetView() // Correctly get the map instance
